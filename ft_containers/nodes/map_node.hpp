...
/*   Updated: 2021/07/09 03:34:03 by seyu             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
public:
	...
	map_node()
		: value(NULL), _color(BLACK), _allocValue(allocator_type()), _allocNode(std::allocator<map_node>())
	{
		_parent = this;
		_left = this;
		_right = this;
	...
	map_node(const value_type& val, map_node* const nil, const allocator_type& alloc = allocator_type())
		: _color(RED), _parent(nil), _left(nil), _right(nil), _allocValue(alloc), _allocNode(std::allocator<map_node>())
	{
	...
	map_node*	find(const T& target)
	{
		...
		else
		{
			...
			{
				if (itr->value->first == target.first)
					return (itr);
	...
	map_node*	insert(const value_type& val, map_node* nil)
	{
		...
		_allocNode.construct(new_node, map_node(val, nil));
	...
	void	insert_fixup()
	{
		// if) this == root OR this->parent is BLACK map_node
		if (this->_is_root() || _parent->_color == BLACK)
			return ;
		// this			: RED map_node, have parent node
		// this->parent	: RED map_node
		...
		// this			: RED map_node, have parent node
		// this->parent	: RED map_node, have parent node

		map_node*	p_sibling = _parent->_sibling();
	...
	void	erase()
	{
		...
		{
			map_node*	prev_node = this->prev();
			map_node*	next_node = this->next();
...
private:
			...
			_depth()
	{
		...
		for (map_node* itr = this; itr->_parent->_is_nil() == false; itr = itr->_parent, depth++) ;
		return (depth);
	}
	...
	void	_connect(map_node *left, map_node *right)
	{
		if (left != NULL)
		{
	...
	void	_rotate()
	{
		...
		map_node*	gparent = _parent->_parent;
	...
	std::allocator<map_node>	_allocNode;
};
