...
/*   Updated: 2021/05/01 21:09:35 by seyu             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int
	gnl_check_param(int fd, char **line)
{
	if (read(fd, NULL, 0) < 0)
		return (1);
	if (line == NULL)
...
static void
	gnl_strjoin(char **line, char **buffer, size_t next_line_size)
{
	char	*join_line;
	char	*dup_buffer;
...
static int
	gnl_read_buffer(char **buffer, char **line)
{
	ssize_t	itr;
	int		rtn;
...
static int
	gnl_read_file(int fd, char **buffer)
{
	ssize_t	read_rtn;
